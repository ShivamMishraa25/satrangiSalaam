...
if (isset($_POST['submit'])) {
    ...
    if ($stmt->execute()) {
        ...
        try {
            // SMTP configuration
            $mail->isSMTP();
            $mail->Host = 'smtp.satrangisalaam.in'; // Your SMTP host
            $mail->Port = 465; // Port number for SSL
            $mail->SMTPAuth = true;
            $mail->Username = 'notification@satrangisalaam.in'; // Your email address
            $mail->Password = 'H,,Z%B`EA1*3'; // Your email account password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS; // Use SSL encryption

            // Email content
            $mail->setFrom('notification@satrangisalaam.in', 'Satrangi Salaam Notifications');
            $mail->addAddress('bhartenduvimaldubey@gmail.com', 'Bhartendu Vimal Dubey'); // Replace with moderator's email

            $mail->isHTML(true);
            $mail->Subject = 'New Submission Received';
            $mailBody = "<h2>New Submission Received</h2>";
            $mailBody .= "<b>Name:</b> $name<br>";
            $mailBody .= "<b>Preferred Name:</b> $preferredName<br>";
            $mailBody .= "<b>Pronouns:</b> $pronouns<br>";
            $mailBody .= "<b>Father's Name:</b> $fatherName<br>";
            $mailBody .= "<b>Post:</b> $post<br>";
            $mailBody .= "<b>Reference:</b> $reference<br>";
            $mailBody .= "<b>Address:</b> $address<br>";
            $mailBody .= "<b>Occupation:</b> $occupation<br>";
            $mailBody .= "<b>Mobile No:</b> $mobileNo<br>";
            $mailBody .= "<b>Email:</b> $email<br>";
            $mail->Body = $mailBody;
