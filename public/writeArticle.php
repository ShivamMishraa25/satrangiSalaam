...
<meta property="og:title" content="Write Article">
<meta property="og:description" content="Read and Write Articles that will be dsiplayed on website for everyone to read. Share your thoughts with the World.">
    ...
    <body>
        ...
        <img id="bird2" src="../img/bird2.png" alt="flying bird">
          <section class="content">
            <h1 class="section-title">आर्टिकल लिखें व पढ़ें</h1>
          <a href="../includes/article_form" class="btn-highlight">अपना आर्टिकल लिखें</a>

                <?php
include '../includes/db.php'; // Include your database connection

// Fetch articles ordered by the latest first
$sql = "SELECT * FROM articles WHERE status = 'approved' ORDER BY created_at DESC";
$result = mysqli_query($conn, $sql);

echo "<h2 class='section-title'>विभन्न लेखकों द्वारा लिखे गये आर्टिकल्स</h2>";

if (mysqli_num_rows($result) > 0) {
while ($row = $result->fetch_assoc()) {
    echo "<div class='article'>";
    echo "<h3>" . htmlspecialchars($row['title']) . "</h3>"; // Article title
    echo "<p><strong>By: </strong>" . htmlspecialchars($row['name']) . "</p>"; // Writer's name
    echo "<p>" . htmlspecialchars($row['content']) . "</p>"; // Article content
    echo "<p><strong>Contact: </strong>" . htmlspecialchars($row['contact']) . "</p>"; // Writer's contact
    echo "<small>Posted on: " . $row['created_at'] . "</small>";
    echo "</div><hr>";
} }else {
    echo "<p>No articles have been approved yet.</p>";
}
?>
          </section>
        <footer>
